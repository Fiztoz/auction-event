...
module Basic4::ProductAuction::Application::ListClosedAuctions
  ...
  # Admin read: every closed (ended or completed) auction across all sellers,
  # most-recently ended first. A read with no failure mode, so it returns a plain
  ...
  def call(container: Basic4::Container.production)
    container[:product_repository].find_closed
  end
end
